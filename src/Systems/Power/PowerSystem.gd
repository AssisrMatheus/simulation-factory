...
func _on_systems_ticked(delta: float) -> void:
	...
	for path in paths:
		...
		var current_power_receivers: Array = path.slice(1, path.size() - 1)
		for cell in current_power_receivers:
			...
			if not receivers_already_provided.has(cell):
				receivers_already_provided[cell] = received_power
			else:
				receivers_already_provided[cell] += received_power
